...
var stringVacio = ""
var cadenaVacia = String() // cadena vacia, al constructor del tipo.


// CONTAR UNA CADENA VACIA
stringVacio.count   // 👈 me dice el numero de caracteres que hay en la cadena con la propiedad '.count'
stringVacio.isEmpty  // 👈 me dice si la cadena esta vacia con la propiedad '.isEmpty' y me devuelve un Bool.
...
// PREFIJOS Y SUFIJOS
/*La función hasSuffix es un método que se utiliza en las cadenas de Swift para verificar si una cadena termina con una determinada subcadena. Este método devuelve un valor booleano, true si la cadena termina con la subcadena especificada y false si no.*/
...
/* 👆Estos métodos son útiles cuando necesitas realizar acciones específicas basadas en si una cadena tiene un cierto prefijo o sufijo. Por ejemplo, podrías usar hasSuffix para identificar tipos de archivos en función de su extensión (por ejemplo, ".jpg", ".png") o hasPrefix para buscar ciertos códigos de país en números de teléfono.*/
...
inicio1.hasSuffix(topico) && inicio2.hasSuffix(topico) // aqui combinamos las dos condiciones si son verdaderas con un &&.
...
inicio.contains("lejano") //  si existe la palabara "lejano" -aqui con la condicion '.contains' contine un valor en la cadena "lejano" - tambien podemos poner si contiene una cadena un valor(un caracter) o una cadena completa
inicio.contains("anillo")  // si existe la palabra "anillo"


//CADENA POR RANGO
if let rango = inicio.range(of: "pais"){     // con esto podemos buscar o extraer un valor dentro de una subcadena.se que no la vais a escuchar è
